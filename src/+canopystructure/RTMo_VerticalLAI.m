...
tau = repmat(leafopt.tran',nl,1);                  % [nl,nwl]   leaf tansmition
kChlrel = repmat(leafopt.kChlrel',nl,1);           % [nl,nwl]   Chlorophyll AB content
kCarrel = repmat(leafopt.kCarrel',nl,1);           % [nl,nwl]   Carotenoid content
...
cos_tts     = cos(tts*deg2rad);             %           cos solar       angleffcone
...
rad.Pnh_Car = 1E6*Pnhc_Car;% [60x1 double]      net PAR absorbed by Car (moles m-2 s-1) of shaded leaves
rad.Pnu_Car = 1E6*Pnuc_Car; % [13x36x60 double] net PAR absorbed by Car (moles m-2 s-1) of sunlit leaves
...
if ~isfield(atmo,'Esun_')
    ...
    Esun_   = max(1E-6,pi*t1.*t4);                                                        % Sun irradiance onto ground surface; Eq.5 in Van der Tol. (2009)
    Esky_   = max(1E-6,pi./(1-t3.*rdd).*(t1.*(t5+t12.*rsd)+Fd+(1-rdd).*Ls.*t3+t16));      % Solar irradiance onto ground surface; Eq.6 and 7 in Van der Tol. (2009)
